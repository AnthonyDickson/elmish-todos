// This dependency is vendored to apply the fix in https://github.com/Zaid-Ajaj/Feliz.Router/pull/72 without waiting for upstream to merge
    ...
    let router =
        React.memo (fun (input : RouterProps) ->
            let useCallbackRef (callback : 'T -> unit) =
                ...
                React.useEffect ((fun () -> callbackRef.current <- callback), [| callback :> obj |])

                React.useCallback ((fun arg -> callbackRef.current arg), [||])

            let onChange =
                useCallbackRef (fun (ev : Event) ->
...
type Cmd =
    ...
    static member inline navigatePath
        (segment1 : string, segment2 : string, value1 : int, mode : HistoryMode)
        : Cmd<'Msg> =
    ...
    static member inline navigatePath
        (segment1 : string, value1 : int, segment2 : string, mode : HistoryMode)
        : Cmd<'Msg> =
